...
@section('content')
    ...
    <div class="mt-4 rounded-2xl border border-slate-200">
        <div class="border-b border-slate-200 bg-slate-50/70 px-4 py-3">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <p class="text-sm font-semibold text-slate-700">Bộ lọc dự án</p>
                        ...
                        <div>
                            ...
                            <select id="status" name="status"
                                ...
                                @foreach($statuses as $status)
                                    <option {{ request('status') === $status->value ? 'selected' : '' }} value="{{ $status }}">{{ $status->label() }}</option>
                                @endforeach
                            </select>
                        </div>
                        <button type="submit"
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    @forelse($projects as $project)
                        <tr class="group hover:bg-slate-50">
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-600">{{ $project->client->customer_name }}
                            </td>
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-600">
                                {{ $project->deadline->format('d-m-Y') }}</td>
                            <td class="whitespace-nowrap px-4 py-3">
    ...
    <div class="mt-4 flex flex-wrap items-center justify-between gap-3">
        @if($projects->total() > 0)
            <div class="text-sm text-slate-600">Hiển thị {{ $projects->firstItem() }}-{{ $projects->lastItem() }} /
                {{ $projects->total() }}</div>
        @else
