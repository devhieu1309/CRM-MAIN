...
@section('content')
    <form action="{{ route('projects.update', $project) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <input type="text" name="deadline" value="{{ old('deadline', $project->deadline->format('d/m/Y')) }}"
    ...
    <section class="mt-8 rounded-2xl border border-slate-200 bg-slate-50/50 shadow-sm">
        @if (session('status'))
            ...
            >
                <span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-emerald-500 text-white shadow-sm">
        ...
        <div class="space-y-6 bg-white px-6 py-5">
            <form action="{{ route('media.upload', ['model' => 'Project', 'id' => $project->id]) }}" method="POST"
                enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
                    <label for="project-file" class="text-sm font-semibold text-slate-700">File</label>
                    <input id="project-file" type="file" name="file"
                            ...
                            <tbody class="divide-y divide-slate-200 bg-white">
                                ...
                                <tr class="transition-colors hover:bg-slate-50/80">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-600">
                                        <time datetime="2026-05-23T14:30:00">{{ $media->created_at->format('d/m/y H:i') }}</time>
                                    </td>
                                    <td class="whitespace-nowrap px-4 py-3 text-right">
                                        <div class="inline-flex items-center justify-end gap-2">
                                            ...
                                            <form action="{{ route('media.delete', ['model' => 'Project', 'id' => $project->id, 'media' => $media]) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button onclick="return confirm('Bạn có chắc muốn xóa file này không?')" type="submit"
