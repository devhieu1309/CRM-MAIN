...
@section('page_title', 'Thêm công việc')
@section('page_desc', 'Tạo mới một công việc')
    ...
    <form action="{{ route('tasks.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="sm:col-span-2">
                ...
                <input name="title" type="text" vlalue="{{ old('title') }}" placeholder="Nhập tiêu đề công việc"
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="description" rows="4" placeholder="Nhập mô tả công việc"
            ...
            <div>
                ...
                <select name="client_id"
                    ...
                    @foreach($clients as $client)
                        <option {{ old('client_id') == $client->id ? "selected" : "" }} value="{{ $client->id }}">
            ...
            <div>
                ...
                <select name="user_id"
                    ...
                    @foreach($users as $user)
                        <option {{ old('user_id') == $user->id ? "selected" : "" }} value="{{ $user->id }}">{{ $user->name }}
                        </option>
                    @endforeach
                </select>
                @error('client_id')
            ...
            <div>
                ...
                <input type="text" value="{{ old('deadline') }}" name="deadline" placeholder="mm/dd/yyyy"
                    class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-600/20">
                @error('deadline')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
